...
const ACTIVE_HOUR_START = 8;  // heure UTC de début de fenêtre active
const ACTIVE_HOUR_END = 22;   // heure UTC de fin de fenêtre active
...
function isWithinActiveHours() {
    ...
    return hourUTC >= ACTIVE_HOUR_START && hourUTC < ACTIVE_HOUR_END;
}
...
async function runScan() {
    if (!isWithinActiveHours()) {
        console.log(`[${new Date().toISOString()}] Hors fenêtre active (${ACTIVE_HOUR_START}h-${ACTIVE_HOUR_END}h UTC), scan ignoré.`);
        return;
    }
...
app.get('/', (req, res) => {
    res.json({
        ...
        paires: PAIRS,
        fenetre_active: `${ACTIVE_HOUR_START}h-${ACTIVE_HOUR_END}h UTC`,
...
app.listen(PORT, () => {
    ...
    console.log(`Fenêtre active: ${ACTIVE_HOUR_START}h-${ACTIVE_HOUR_END}h UTC, scan toutes les ${SCAN_INTERVAL_MS / 60000} min`);
